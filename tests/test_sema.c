...
static void test_shapes(void)
{
    ...
    /* Столбец и строка дают таблицу; строка растягивается по матрице — и в
     * обратную сторону тоже: форму результата задаёт больший по каждой оси. */
    expect_clean("*&c<f32:3,1> -> @alloc => $c;\n"
                 "*&r<f32:1,4> -> @alloc => $r;\n"
                 "$c -> @add($r) => *&T<f32:3,4>;\n"
                 "$r -> @sub($c) => *&U<f32:3,4>;\n"
                 "*&T -> @mul($r) -> @div($c) => *&V<f32:3,4>;", "столбец и строка");
    expect_clean("*&M<i32:2,3> -> @alloc => $m;\n"
                 "*&r<i32:1,3> -> @alloc => $r;\n"
                 "$m -> @sub($r) -> @reduce.add => $s;", "матрица и строка, свёртка после");
    expect_err("*&c<f32:3,1> -> @alloc => $c;\n"
               "*&r<f32:1,4> -> @alloc => $r;\n"
               "$c -> @add($r) => *&T<f32:3,1>;", SMP_E0303, "приёмник меньше результата");
    expect_err("*&A<f32:2,3> -> @alloc => $a;\n"
               "*&B<f32:3,2> -> @alloc => $b;\n"
               "$a -> @add($b) => $c;", SMP_E0309, "оси не 1 и не равны");
    expect_err("*&A<f32:2,3> -> @alloc => $a;\n"
               "*&v<f32:3> -> @alloc => $v;\n"
               "$a -> @add($v) => $c;", SMP_E0309, "ранги не равны — не растягивается");
    expect_err("*&c<f32:3,1> -> @alloc => $c;\n"
               "$c -> @add(2.0) => $d;", SMP_E0309, "число с тензором — по-прежнему нет");
}
