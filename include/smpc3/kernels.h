 ...
 * и внимание: q x K^T по кэшу ключей. B в q8_0 распаковывается на лету, в
 * регистрах; A и C тогда f32. B в f32 со строками подряд (шаг между ними
 * любой — голова KV-кэша) считается скалярными произведениями строк на
 * AVX2; прочее — обычный GEMM по транспонированному виду. Векторная ветка
 * делит строки B между потоками рабочей памяти (scratch->sets): каждый выход
 * считает ровно один поток в одном и том же порядке, так что результат бит
 * в бит тот же, что на одном потоке. */
void smp_k_mmul_t(const SmpBuf *c, const SmpBuf *a, const SmpBuf *b,
                  SmpKScratch *scratch);

/* Ниже этого M*N*K раздача строк B потокам не окупается. */
void smp_k_mmul_t_par_min(uint64_t work);
