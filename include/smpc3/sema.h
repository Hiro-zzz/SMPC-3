...
#define SMP_OPS(X)                                                             \
    ...
    X(MMUL,       "mmul",       1, 2, "[M,K]x[K,N]; второй — живая длина оси K") \
    X(MMUL_T,     "mmul.t",     1, 2, "[M,K]x[N,K]^T; второй — живых строк B")  \
    ...
    X(ABS,        "abs",        0, 0, "поэлементно |x|; и над скаляром")       \
    X(SQRT,       "sqrt",       0, 0, "корень: f32, f64; и над скаляром")      \
    X(SCALE,      "scale",      1, 1, "умножить поэлементно на скаляр")        \
    X(ADD,        "add",        1, 1, "сложение: тензоров поэлементно или чисел") \
    X(MUL,        "mul",        1, 1, "умножение: тензоров поэлементно или чисел") \
    X(SUB,        "sub",        1, 1, "вычитание: тензоров поэлементно или чисел") \
    X(DIV,        "div",        1, 1, "деление: тензоров f32/f64 или чисел")   \
    X(SILU,       "silu",       0, 0, "поэлементно x / (1 + e^-x)")            \
    X(RMSNORM,    "rmsnorm",    1, 1, "RMS-нормировка строк; аргумент — eps")  \
    X(SOFTMAX,    "softmax",    0, 1, "softmax строк; аргумент — длина")       \
    X(ROPE,       "rope",       2, 2, "поворот RoPE: позиция и основание")     \
    X(ARGMAX,     "argmax",     0, 0, "номер наибольшего элемента")            \
    X(RESHAPE,    "reshape",    1, 4, "та же память в другой форме")           \
    ...
    X(CAST_Q8_0,  "cast.q8_0",  0, 0, "квантовать в q8_0 блоками по 32")       \
