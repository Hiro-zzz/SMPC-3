...
typedef struct SmpDiagCtx {
    ...
    /* Коротко, для модели, которая пишет программы: код, место, строка
     * исходника, ОШИБКА, ДЕТАЛИ, ИСПРАВЛЕНИЕ. Без рамки и каретки, без
     * ДИАГНОЗ и без дампа регистров — это человеку, а модели они съедают
     * контекст и ничего не говорят о том, что чинить. */
    bool             compact;
    ...
    /* Коды, которые хозяин попросил не показывать (smp_diag_mute). */
    uint64_t         muted[(SMP_DIAG__COUNT + 63u) / 64u];
...
/* Не показывать этот код: он не печатается и не считается. Для хозяина, у
 * которого предупреждение заведомо неверно, — калькулятор показывает
 * человеку каждый регистр, и W0301 «записано и не прочитано» там ложь.
 * Только предупреждения и заметки: фатальную ошибку заглушить нельзя, и
 * на такую просьбу ответ — false. */
